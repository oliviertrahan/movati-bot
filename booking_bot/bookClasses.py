...
with dockerfile_image.imports():
    ...
    for user_to_book in accounts_to_book:
        for booking_config in user_to_book.class_bookings:
            ...
            class_configs_per_location[booking_config.location_id].append(
                (
                    user_to_book.person_name,
                    user_to_book.account_email,
                    user_to_book.account_password,
                    booking_config,
                )
            )
    ...
    def get_valid_date_info(date_string):
        ...
        match = re.search(
            rf'({"|".join(valid_days_to_book)}), ([a-zA-Z]+) (\d+), (\d+)', date_string
        )
        if match is None:
            return None
    ...
    def get_class_state(class_details):
        ...
        if match is not None:
            ...
            match = re.search(
                r'start-info="(\d{2})/(\d{2})/(\d{4}) at (\d{1,2})\:(\d{2})(a|p)m"',
                class_details,
            )
            if match is None:
            ...
            if meridian == "p" and hour != 12:
                hour += 12
            can_start_booking_on = datetime(
                year, month, day, hour, minutes, 0, 0, movati_trainyards_timezone
            )
    ...
    def get_class_booking_url(class_details):
        match = re.search(
            r"(https://groupexpro.com/gxp/reservations/schedule/index/\d+/\d{2}/\d{2}/\d{4})",
            class_details,
        )
    ...
    def book_class(booking_url, user_email, user_password):
        ...
        # Now we login
        data = {
            "dest": login_page.replace(
                "https://groupexpro.com/gxp/auth/login/", ""
            ).replace("?c=1&e=1&type=new", ""),
            "type": "new",
            "e": "1",
            "embeddedID": "",
    ...
    def book_classes():
        ...
        for _, (location_id, user_bookings) in enumerate(
            class_configs_per_location.items()
        ):
            ...
            response_json = re.sub(
                r"\\\s", "", response_json
            )  # replace single backslash with empty string
            response_json = re.sub(
                r"\\\'", "'", response_json
            )  # replace single backslash and quote with quote
            ...
            for movati_class_data in schedule["aaData"]:
                class_data = {
                    ...
                    for key, value in enumerate(movati_class_data)
                }
                ...
                for (
                    ...
                    is_valid_class = (
                        booking_config.class_name.lower() in class_data["name"].lower()
                    )
                    ...
                    class_datetime = datetime(
                        ...
                        class_time.hour,
                        class_time.minute,
                        tzinfo=movati_trainyards_timezone,
                    )

                    is_valid_class_time = is_valid_time(
                        class_time, booking_config.start_time, booking_config.end_time
                    )
                    class_state, spots_available, can_start_booking_on = (
                        get_class_state(class_data["details"])
                    )

                    # Book class if it is valid and has unbooked spots
                    if (
                        is_valid_class
                        and class_day_of_week == booking_config.day_of_week
                        and is_valid_class_time
                    ):
                        class_dict_key = get_class_dict_key(class_data)
                        # class_present = class_want_to_book[class_dict_key]
                        class_present = False
                        if class_present:
                            check_expiration = class_want_to_book.get_expiration(
                                class_dict_key
                            )
                            print(
                                f"Found valid class for {person_name}: {class_data['name']} on {class_day_of_week} at {class_data['time_range']} which we have already tried/succeeded to book, but will only be available on {check_expiration}. Waiting."
                            )
                            continue
                        ...
                        if spots_available > 1:
                            ...
                            if can_start_booking_on and now_time < can_start_booking_on:
                                print(
                                    f"Found valid class for {person_name}: {class_data['name']} on {class_day_of_week} at {class_data['time_range']} which is not yet available to book. Waiting to book until {can_start_booking_on}."
                                )
                                continue

                            # Handle scenario where the class is too soon, we don't want to book that close
                            print(
                                f"now_time + timedelta(hours=15) : {now_time + timedelta(hours=15) }"
                            )
                            if now_time + timedelta(hours=15) > class_datetime:
                                print(
                                    f"Found valid class for {person_name}: {class_data['name']} on {class_day_of_week} at {class_data['time_range']} which is available to book but is too soon! Will not book."
                                )
                                continue

                            # get booking URL to book
                            print(
                                f"Found valid class for {person_name}: {class_data['name']} on {class_day_of_week} at {class_data['time_range']} which is not fully booked. Booking now..."
                            )
                            ...
                            booked = book_class(
                                booking_url, user_email, user_password
                            )  # TODO: handle case where different accounts have different states
                            ...
                            class_want_to_book.set_expiration(
                                class_dict_key,
                                datetime.now(tz=movati_trainyards_timezone)
                                + timedelta(hours=3),
                            )
                            booked_classes.append(
                                (
                                    class_data["name"],
                                    class_data["time_range"],
                                    class_day_of_week,
                                    person_name,
                                    booked,
                                )
                            ...
                            print(
                                f"Found valid class for {person_name}: {class_data['name']} on {class_day_of_week} at {class_data['time_range']} with state \"{class_state}\". Skipping..."
                            )
                    else:
                        pass
            ...
            print(
                f"Found {len(booked_classes)} classes to book for location {movati_trainyards_location_id}"
            )
            for (
                class_name,
                class_time,
                ...
                if booked:
                    print(
                        f"Booked {class_name} at {class_time} on {class_day_of_week} for {person_name}!"
                    )
                else:
                    print(
                        f"Wanted to book {class_name} at {class_time} on {class_day_of_week} for {person_name}, but it was already done!"
                    )
