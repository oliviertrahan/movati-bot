...
if stub.is_inside():
    ...
    start = floor((datetime.now() + timedelta(days=4)).timestamp()) #start time to get schedule
...
def get_valid_day(date_string, day_to_book):
    match = re.search(rf'({day_to_book})', date_string)
    ...
    for _, (location_id, user_bookings) in enumerate(class_configs_per_location.items()):
        ...
        for movati_class_data in schedule['aaData']:
            ...
            for person_name, user_email, user_password, booking_config in user_bookings:
                ...
                valid_class_date = get_valid_day(class_data['date'], booking_config.day_of_week)
                ...
                if (is_valid_class and valid_class_date is not None and is_valid_class_time):
                    ...
                    if class_present:
                        ...
                        print(f"Found valid class for {person_name}: {class_data['name']} on {valid_class_date} at {class_data['time_range']} which we have already tried/succeeded to book, but will only be available on {check_expiration}. Waiting.")
                        continue
                    ...
                    if (spots_available > 1):
                        ...
                        if (can_start_booking_on and datetime.now(tz=movati_trainyards_timezone) < can_start_booking_on):
                            print(f"Found valid class for {person_name}: {class_data['name']} on {valid_class_date} at {class_data['time_range']} which is not yet available to book. Waiting to book until {can_start_booking_on}.")
                            continue

                        #get booking URL to book
                        print(f"Found valid class for {person_name}: {class_data['name']} on {valid_class_date} at {class_data['time_range']} which is not fully booked. Booking now...")
                        ...
                        booked_classes.append((class_data['name'], class_data['time_range'], valid_class_date, person_name, booked))
                    else:
                        print(f"Found valid class for {person_name}: {class_data['name']} on {valid_class_date} at {class_data['time_range']} with state \"{class_state}\". Skipping...")
                    
        print(f"Found {len(booked_classes)} classes to book for location {movati_trainyards_location_id}")
        for class_name, class_time, valid_class_date, person_name, booked in booked_classes:
            if booked:
                print(f"Booked {class_name} at {class_time} on {valid_class_date} for {person_name}!")
            else:
                print(f"Wanted to book {class_name} at {class_time} on {valid_class_date} for {person_name}, but it was already done!")
